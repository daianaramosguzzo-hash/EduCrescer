...
export const STAT_DESC = {
  ...
  velocidade: 'Pontos de Ação por turno.',
...
export const SKILLS = {
  sumir: { nome: 'Sombra Mirim', hero: 'arthur', ativa: true, pa: 1, recarga: [5, 4, 3], icon: '🥷',
    desc: r => `Ativa (1 PA): some das vistas na hora, mesmo sem esconderijo, se nenhum inimigo estiver colado. Recarga de ${[5, 4, 3][r - 1]} turnos. Passiva: +${r} de Furtividade.` },
  ...
  estilingada: { nome: 'Estilingada Certeira', hero: 'arthur', icon: '🎯',
    desc: r => `Passiva: +${[15, 25, 35][r - 1]}% de precisão com estilingue e arremessos.${r >= 2 ? ' A primeira pedrada de cada turno sempre atordoa.' : ''}${r >= 3 ? ' Distrações vão duas vezes mais longe.' : ''}` },
  maos_que_curam: { nome: 'Mãos que Curam', hero: 'carol', icon: '🩹',
    desc: r => `Passiva: medicamentos curam ${[30, 60, 90][r - 1]}% a mais e custam 1 PA a menos.${r >= 3 ? ' Levantar um aliado caído custa só 2 PA.' : ''}` },
  fe_inabalavel: { nome: 'Fé Inabalável', hero: 'carol', ativa: true, pa: 2, recarga: [6, 5, 4], icon: '🙏',
    desc: r => `Ativa (2 PA): palavra de ânimo, +${[15, 25, 35][r - 1]} de moral para quem estiver a até 5 casas e fim do pânico. Passiva: o moral da Carol cai pela metade.` },
  ...
  voz_de_comando: { nome: 'Voz de Comando', hero: 'daiana', ativa: true, pa: 2, recarga: [1, 1, 1], icon: '📣',
    desc: r => `Ativa (2 PA): "Organiza a turma!" Um aliado a até 6 casas ganha +${[2, 3, 4][r - 1]} PA neste turno. Uma vez por turno.` },
  plano_de_aula: { nome: 'Plano de Aula', hero: 'daiana', icon: '📋',
    desc: r => `Passiva: vasculhar custa 1 PA a menos.${r >= 2 ? ' Ao entrar num prédio, a planta dele aparece no mapa.' : ''}${r >= 3 ? ' O grupo ganha 10% a mais de experiência.' : ''}` },
  ...
  surto: { nome: 'Surto Nervoso', hero: 'pablicio', ativa: true, pa: 0, recarga: [8, 7, 6], icon: '😤',
    desc: r => `Ativa (0 PA): +${[3, 4, 5][r - 1]} PA e +50% de dano corpo a corpo neste turno. Depois, −15 de moral e de energia. Passiva: com menos de 40% de vida, +25% de dano.` },
  mao_na_graxa: { nome: 'Mão na Graxa', hero: 'pablicio', icon: '🔧',
    desc: r => `Passiva: abre fechaduras sem barulho (2 PA), conserta veículos, geradores e rádios; barricadas custam 1 PA a menos.${r >= 2 ? ' Armas brancas gastam metade da durabilidade.' : ''}${r >= 3 ? ' Improvisa peças com sucata.' : ''}` },
  couro_grosso: { nome: 'Couro Grosso', hero: 'pablicio', ativa: true, pa: 1, recarga: [3, 3, 2], icon: '🛡️',
    desc: r => `Passiva: −${[10, 20, 30][r - 1]}% de dano recebido. Ativa (1 PA): "Pode vir!" Zumbis próximos passam a atacar o Pablício neste turno.` },
};
...
export const PERKS = {
  ...
  maratonista: { nome: 'Maratonista', desc: '+1 PA máximo.' },
